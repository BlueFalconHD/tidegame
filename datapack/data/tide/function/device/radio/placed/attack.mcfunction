...
execute if score #radio_pickup_allowed tide.device.tmp matches 1 at @s as @e[type=minecraft:item_display,tag=tide.device.radio.display,distance=..2] if score @s tide.device.id = #radio_id tide.device.id at @s run function tide:device/radio/placed/pickup
tag @a remove tide.device.radio.picker
