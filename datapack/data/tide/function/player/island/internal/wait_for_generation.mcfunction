...
execute in minecraft:overworld as @e[type=minecraft:marker,tag=tide.island.registry] if score @s tide.identifier.island = #island_id tide.identifier.island run scoreboard players operation #island_state tide.island.tmp = @s tide.island.state
