...
execute as @e[type=minecraft:bamboo_raft,tag=tide.raft.registered,distance=..2,sort=nearest,limit=1] if score @s tide.identifier.island = #picker_island tide.identifier.island at @s run function tide:island/raft/internal/pickup
