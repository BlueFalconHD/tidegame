...
execute in minecraft:overworld as @e[type=minecraft:marker,tag=tide.island.registry,tag=!tide.island.destroying] if score @s tide.identifier.island = #repair_island tide.identifier.island store result score #repair_prepared tide.raft.tmp run function tide:item/raft/repair/internal/prepare
