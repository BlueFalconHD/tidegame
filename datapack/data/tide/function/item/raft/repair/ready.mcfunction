...
execute if score #repair_interaction_found tide.raft.tmp matches 1 as @e[type=minecraft:bamboo_raft,tag=tide.raft.repair_target,limit=1] at @s run function tide:item/raft/internal/ensure_interaction
...
execute if score #repair_raft_found tide.raft.tmp matches 1 unless score #repair_interaction_found tide.raft.tmp matches 1 as @e[type=minecraft:bamboo_raft,tag=tide.raft.repair_target,limit=1] at @s store result score #repair_interaction_created tide.raft.tmp run function tide:item/raft/internal/create_interaction
...
execute unless score #repair_raft_found tide.raft.tmp matches 1 in minecraft:overworld as @e[type=minecraft:marker,tag=tide.island.registry] if score @s tide.identifier.island = #repair_island tide.identifier.island run function tide:item/raft/internal/unregister_island
...
execute unless score #repair_raft_found tide.raft.tmp matches 1 if score #repair_player_online tide.raft.tmp matches 1 as @a if score @s tide.identifier.player = #repair_player tide.identifier.player at @s run loot spawn ~ ~ ~ loot tide:item/raft
execute unless score #repair_raft_found tide.raft.tmp matches 1 unless score #repair_player_online tide.raft.tmp matches 1 run loot spawn ~ ~ ~ loot tide:item/raft
