...
execute in minecraft:overworld as @e[type=minecraft:marker,tag=tide.worldgen.map.controller,tag=tide.worldgen.map.accepted] if score @s tide.identifier.structure = #structure_id tide.identifier.structure in tide:ocean store result score #dispatch tide.worldgen.map.tmp run function tide:worldgen/map/structure/build/dispatch
