...
execute unless score #social_friend_exists tide.social.tmp matches 1 run tellraw @s {"text":"You are not friends with that player.","color":"red"}
...
execute as @a if score @s tide.identifier.player = #social_target tide.social.tmp run tellraw @s [{"selector":"@a[tag=tide.social.actor,limit=1]","color":"aqua"},{"text":" removed you as a friend.","color":"gray"}]
