...
data modify storage tide:social ui.dialog set value {type:"minecraft:multi_action",title:{text:"Friend Request",color:"gold"},body:[],can_close_with_escape:true,pause:false,after_action:"close",actions:[],columns:2,exit_action:{label:{text:"Back"},width:150,action:{type:"run_command",command:"trigger tide.social.menu set 2"}}}
