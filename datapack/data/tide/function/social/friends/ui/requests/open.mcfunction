...
data modify storage tide:social ui.dialog set value {type:"minecraft:multi_action",title:{text:"Friend Requests",color:"#F79317"},body:[{type:"minecraft:plain_message",contents:{text:"Incoming and sent friend requests.",color:"#F2F2F2"},width:260}],can_close_with_escape:true,pause:false,after_action:"close",actions:[],columns:1,exit_action:{label:{text:"Back",color:"#F2F2F2"},width:150,action:{type:"run_command",command:"trigger tide.social.menu set 4"}}}
...
execute unless score #social_ui_entries tide.social.tmp matches 1.. run data modify storage tide:social ui.dialog.body append value {type:"minecraft:plain_message",contents:{text:"No pending requests.",color:"#F2F2F2"},width:260}
