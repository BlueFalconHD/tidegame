...
data modify storage tide:social ui.dialog set value {type:"minecraft:multi_action",title:{text:"Add Friend",color:"#E0CA8E"},body:[{type:"minecraft:plain_message",contents:{text:"Choose an online player.",color:"#F2F2F2"},width:260}],can_close_with_escape:true,pause:false,after_action:"close",actions:[],columns:1,exit_action:{label:{text:"Back",color:"#F2F2F2"},width:150,action:{type:"run_command",command:"trigger tide.social.menu set 4"}}}
execute as @a[tag=!tide.social.viewer] run function tide:social/friends/ui/add/prepare
execute unless score #social_ui_entries tide.social.tmp matches 1.. run data modify storage tide:social ui.dialog.body append value {type:"minecraft:plain_message",contents:{text:"No available online players.",color:"#F2F2F2"},width:260}
